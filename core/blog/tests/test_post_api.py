...
@pytest.fixture
def common_user():
    user = User.objects.create_user(email="admin@admin.com",password="@#123456",is_verified=True)
    return user

@pytest.fixture
def post(db,common_user):
    return Post.objects.create(author=common_user,title="Existing Post",status=True,published_date=datetime.now())
...
class TestPostApi():
    ...
    ################################################
    def test_partial_update_post_response_200_status(self,api_client,common_user,post):
        url = reverse('blog:api-v1:post-detail', kwargs={'pk':post.id})
        data = {
            "title" : "partial-update-post",           
        }
